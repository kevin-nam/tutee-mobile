...
class Login extends React.Component {
  ...
  logIn = async () => {
    ...
    const { type, token } = await Expo.Facebook.logInWithReadPermissionsAsync('1868009723488146', {
    ...
    if (type === 'success') {
      ...
      const response = await fetch(`https://graph.facebook.com/me?fields=name,email,picture&access_token=${token}`)
        .then(function(response) {
          ...
          AsyncStorage.setItem('@MySuperStore:' + USER_PHOTO_KEY, data.picture.data.url);
